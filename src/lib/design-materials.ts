...
function drawWoodPlank(ctx: CanvasRenderingContext2D, size: number, hex: string) {
  ...
  for (let row = 0; row < 6; row++) {
    const y = row * plankH;
    const offset = row % 2 === 0 ? 0 : plankH * 0.35;
    ctx.fillStyle = shade(hex, row % 2 ? -0.04 : 0.03);
    ctx.fillRect(0, y, size, plankH - 1);
    ctx.strokeStyle = "rgba(0,0,0,0.12)";
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(offset, y + plankH);
    ctx.lineTo(size, y + plankH);
    ctx.stroke();
    for (let gx = 0; gx < size; gx += 3) {
      const n = noise(gx * 0.08, y * 0.05, row);
      ctx.fillStyle = `rgba(0,0,0,${0.02 + n * 0.04})`;
      ctx.fillRect(gx, y + 1, 2, plankH - 2);
    }
  }
}
...
function drawQuartzVeined(ctx: CanvasRenderingContext2D, size: number, hex: string) {
  ...
  for (let i = 0; i < 14; i++) {
    ...
    for (let x = 0; x <= size; x += 8) {
      ctx.lineTo(x, y0 + Math.sin(x * 0.04 + i) * 12 + noise(x, i) * 8);
    }
    ctx.stroke();
  }
  ...
  for (let i = 0; i < 8; i++) {
    ...
    ctx.beginPath();
    ctx.moveTo(x0, 0);
    ctx.lineTo(x0 + noise(i, 4) * 40, size);
    ctx.stroke();
  }
}
...
export function textureCacheKey(kind: TextureKind, hex: string): string {
  return `${kind}:${hex.toLowerCase()}`;
}
...
export function getTextureCanvas(
  ...
  const key = textureCacheKey(kind, hex);
