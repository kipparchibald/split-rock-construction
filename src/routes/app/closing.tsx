...
export const Route = createFileRoute("/app/closing")({ component: ClosingPage });
...
function ClosingPage() {
  const {
    projects, closeoutPackages, realtyDeals, dualRolePolicy,
    setCloseoutItemStatus, setRealtyItemStatus, setRealtyDealStatus, acknowledgeDualCapacity,
  } = useAppStore();
    ...
    <div>
      ...
      <Tabs defaultValue="jobs">
            ...
            return (
              <Card key={projectId}>
                <CardHeader className="flex-row flex-wrap items-start justify-between gap-2">
                  ...
                  <Badge variant={dual.ready ? "success" : "warning"}>{dual.ready ? "Gates clear" : dual.reason}</Badge>
                </CardHeader>
                      ...
                      {!pkg ? (
                        ...
                        <>
                          ...
                          <p className="mt-2 text-[11px] text-fg-muted">
                            G704-style substantial completion is <span className="font-medium text-fg">not</span> a realtor walkthrough.
                          </p>
                      ...
                      {!deal ? (
                        ...
                        <>
                          ...
                          <p className="text-[11px] tabular-nums text-fg-subtle">
                            ...
                            {deal.salePrice ? ` · sale ${formatCurrency(deal.salePrice)}` : deal.listPrice ? ` · list ${formatCurrency(deal.listPrice)}` : ""}
                          ...
                          {deal.status === "listed" ? (
                            <Button size="sm" variant="outline" className="mt-2" onClick={() => setRealtyDealStatus(deal.id, "under_contract")}>
                              Mark under contract
                            </Button>
                          ) : null}
                          {deal.status === "under_contract" ? (
                            <Button size="sm" variant="outline" className="mt-2" onClick={() => setRealtyDealStatus(deal.id, "pending_close")}>
                              Mark pending close
                            </Button>
                          ) : null}
        ...
        <TabsContent value="money" className="grid gap-3 md:grid-cols-2">
          <Card>
            <CardHeader><CardTitle>Construction money</CardTitle></CardHeader>
          ...
          <Card>
            <CardHeader><CardTitle>Realty trust money</CardTitle></CardHeader>
            <CardContent className="space-y-2 text-[13px] text-fg-muted">
              <p>Earnest money and other trust funds → <span className="font-medium text-fg">brokerage trust account only</span>.</p>
          ...
          <Card className="md:col-span-2">
            <CardHeader><CardTitle>Both-gates close rule</CardTitle></CardHeader>
