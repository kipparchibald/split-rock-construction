...
function Dashboard() {
  ...
  return { active, budget, spent, readyDraws, openSafety, commercialCount, attention };
  }, [
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Price the lot, check the plat, bid, then run the job."
        actions={
          <>
            <Button variant="outline" size="sm" asChild>
              <Link to="/app/site-plan">Site plan</Link>
            </Button>
            <Button variant="outline" size="sm" asChild>
              <Link to="/app/field">Field</Link>
            </Button>
            <Button size="sm" asChild>
              <Link to="/app/estimator">Estimate</Link>
            </Button>
          </>
        }
      />
        ...
        <Card className="mb-5 border-dashed">
          <CardContent className="flex flex-col items-start gap-3 py-8 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              <Button size="sm" asChild>
                <Link to="/app/estimator">Estimate a lot</Link>
              </Button>
              <Button size="sm" variant="outline" asChild>
                <Link to="/app/site-plan">Open site plan</Link>
              </Button>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="mb-5 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
        <OperatorQuick title="Estimate" detail="AI draft from lot acres, well, septic, and closed-job history." to="/app/estimator" cta="Price a lot" hot />
        <OperatorQuick title="Site plan / GIS" detail="Recorded Inst. 492361 well, septic, 100' ring, county parcels." to="/app/site-plan" cta="Open plat" hot />
        <OperatorQuick title="Bid board" detail="Turn the draft into a bid the owner can sign." to="/app/bids" cta="Open bids" hot={false} />
        <OperatorQuick
          title="Field"
          detail={
            dailyLogs.filter((l) => l.date === todayIso).length
              ? `${dailyLogs.filter((l) => l.date === todayIso).length} log(s) today`
              : "Logs, photos, and blockers from the lot"
          }
          to="/app/field"
          cta="Field board"
          hot={false}
          ...
          <CardContent className="space-y-0 p-0">
            {attention.length === 0 ? (
              ...
              attention.slice(0, 8).map((a, i) => (
                ...
                >
                  <span className={`mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full ${a.severity === "high" ? "bg-danger" : "bg-warning"}`} />
                ...
                todayLogs.map((l) => {
                  ...
                  return (
                    <div key={l.id} className="border border-border p-3">
                      ...
                      {l.blockers ? <p className="mt-2 text-[11px] text-warning">Blocker: {l.blockers}</p> : null}
                    </div>
                  );
                })
              )}
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              {changeOrders.filter((c) => c.status === "pending_owner").length === 0 ? (
                ...
                changeOrders.filter((c) => c.status === "pending_owner").slice(0, 4).map((c) => {
                  const p = projects.find((x) => x.id === c.projectId);
                  return (
                    <Link key={c.id} to="/app/portal" search={{ project: c.projectId }} className="flex items-start justify-between gap-2 border border-border p-3 transition-colors hover:bg-bg-subtle">
                      <div className="min-w-0">
                        <p className="text-[13px] font-medium">{c.number} · {c.title}</p>
                        <p className="mt-0.5 text-[12px] text-fg-muted">{p?.name} · {formatCurrency(c.amount)}</p>
                      </div>
        ...
        <CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {active.length === 0 ? (
            <div className="col-span-full border border-dashed border-border px-4 py-10 text-center">
              <p className="text-[13px] font-medium text-fg">{isDemoDataEnabled ? "No active jobs on this board." : "No active jobs yet"}</p>
            ...
            active.map((p) => {
              ...
              return (
                <Link key={p.id} to="/app/projects/$projectId" params={{ projectId: p.id }} className="border border-border p-4 transition-colors hover:bg-bg-subtle">
